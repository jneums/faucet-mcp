...
import RequestTokens "tools/request_tokens";
...
shared ({ caller = deployer }) persistent actor class McpServer(
  ...
  // No auth required — this is a public faucet wrapper
  transient let authContext : ?AuthTypes.AuthContext = null;

  // Beacon for usage analytics
  ...
  transient let mcpConfig : McpTypes.McpConfig = {
    ...
    tools = tools;
    toolImplementations = [
      ("request_tokens", RequestTokens.handle(toolContext)),
    ];
      ...
      case (null) {
        if (req.url == "/") {
          return {
            ...
            body = Text.encodeUtf8("<h1>DFINITY Faucet MCP Server</h1><p>Request test tokens (TESTICP / TICRC1) via MCP at <code>/mcp</code></p>");
            upgrade = null;
